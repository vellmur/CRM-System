...
{
    public function __construct($name = 'app:backup')
    {
    ...
    {
        $this->setDescription('Backup of database');
    }
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        $oBackupJob = new CpanelMySQLBackup();
        $oBackupJob->runBackup();

        $output->writeln('Successful backup');
...
class CpanelMySQLBackup
{
    ...
    private $localBackup = 'YES';  // Set this to YES / NO if you want to keep the database backup files locally (where you run this script).
    private $remoteBackup = 'NO';  // Set this to YES/NO if you want to FTP copy your files to another server.

    // CPANEL
    private $cHost = 'blackdirt.org'; // IP address or domain name for the server with the cPanel account
    private $cPort = '2083'; // The port number for the cPanel. If you have problems, try 2082
    private $cUser = 'admin';
    private $cPass = 'w5XbC^S7?!2aDn:P';

    // REMOTE FTP
    private $ftpHost = '';  // IP address or URL of the FTP server
    private $ftpPort = ''; // FTP(S) Port. Default is 21.
    private $ftpUser = '';
    private $ftpPass = '';
    private $ftpPath = ''; // FTP Path (where do you want to copy the files?)
    private $ftpPassive = '';  // Set this to YES to use FTP in passive mode. Necessary if you're having some FTP problems.
    private $ftpSSL = ''; // Set this to YES to use FTPS. Your target FTP server MUST also support this.

    /**
     * CpanelMySQLBackup constructor.
     */
    ...
    public function runBackup()
    {
        ...
        // 2. Open up the FTP connection if needed.
        $fDoFtp = mb_strtoupper($this->remoteBackup) == 'YES';
        $fFtpSuccess = false;

        if ($fDoFtp) {
            $this->ftpPath = rtrim($this->ftpPath, '/') . '/';
            ...
            if (!$fDoFtp && ($this->localBackup != 'YES')) {
        ...
        // 3. For each DB, download it and then copy it to FTP if requested. Then delete if requested.
        for ($i = 0; $i < count($this->databases); $i++) {
            $sDbName = $this->databases[$i];
            ...
            if (mb_strtoupper($this->localBackup) == 'NO') {
    ...
    {
        $oXmlApi = new XmlApi($this->cHost);
        $oXmlApi->password_auth($this->cUser, $this->cPass);
        $oXmlApi->set_port($this->cPort);

        $oResult = $oXmlApi->api2_query($this->cUser, 'MysqlFE', 'listdbs');

        if (!isset($oResult->data[0])) {
            $this->databases[] = (string)($oResult->data->db);
        } else {
            foreach ($oResult->data as $oDatabase) {
                $this->databases[] = (string)($oDatabase->db);
            }
        }
    }
    ...
    {
        $sProtocol = ($this->cPort == '2083') ? 'https://' : 'http://';

        $sLoginUrl = $sProtocol . $this->cHost . ':' . $this->cPort . '/getsqlbackup/' . $insDbName . '.sql.gz';

        $this->writeLog('Download URL: ' . $sLoginUrl);

        $hOut = fopen(dirname(__FILE__) . '/backups/' . $insDbFileName, 'wb');
        ...
        curl_setopt($oCurl, CURLOPT_USERPWD,			$this->cUser . ':' . $this->cPass);
    ...
    protected function openFtpConnection()
    {
        ...
        $sServerAddress = $this->ftpHost;
        $iServerPort = (int)($this->ftpPort);

        // set up FTP connection
        if (($this->ftpSSL == 'YES')) {
            if (function_exists('ftp_ssl_connect')) {
                $this->ftpConnection = ftp_ssl_connect($sServerAddress, $iServerPort);
        ...
        //Fallback
        if (!$this->ftpConnection) {
            $this->ftpConnection = ftp_connect($sServerAddress, $iServerPort);
        }
        ...
        if ($this->ftpConnection) {
            $fLoginResult = ftp_login($this->ftpConnection, $this->ftpUser, $this->ftpPass);
        } else {
            $this->writeLog("Attempt to connect to " . $sServerAddress . ":" . $iServerPort . " failed.");
        }

        // check connection
        if ((!$this->ftpConnection) || (!$fLoginResult)) {
        ...
        if ($fSuccess && ($this->ftpPassive == 'YES')) {
            if (ftp_pasv($this->ftpConnection, true)) {
    ...
    {
        $sDestinationFile = $this->ftpPath . $insDbFileName;

        // upload the file
        $fUpload = ftp_put($this->ftpConnection, $sDestinationFile, $insDbFileName, FTP_BINARY);
        ...
        if (!$fUpload) {
            ...
            $this->writeLog("Uploaded $insDbFileName to " . $this->ftpHost . " as $sDestinationFile");
            $fSuccess = true;
        }
    ...
    /**
     * @param string $insLogData
     * @param bool $infWriteUser
     */
